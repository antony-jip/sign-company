...
function ProjectMaatjeThumb({ maatje, onOpenen }: { maatje: Maatje; onOpenen: (m: Maatje) => void }) {
  ...
  return (
    <button
      ...
      className="group block overflow-hidden rounded-xl bg-card text-left shadow-[0_1px_3px_rgba(0,0,0,0.04)] transition-shadow hover:shadow-[0_4px_16px_rgba(0,0,0,0.08)]"
    >
...
export function ProjectMaatjesTab({ projectId }: ProjectMaatjesTabProps) {
  ...
  return (
    <div className="px-4 py-4 md:px-8 md:py-6">
      {laden ? (
        <p className="py-12 text-center text-[13px] text-muted-foreground">Laden...</p>
      ) : maatjes.length === 0 ? (
        <div className="py-12 text-center">
          <p className="text-[14px] text-muted-foreground">Nog geen maatjes gekoppeld aan dit project.</p>
          <p className="mt-1 text-[13px] text-muted-foreground">Koppel ze vanuit het kladblok onder Maatjes.</p>
        </div>
      ) : (
