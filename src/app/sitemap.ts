import type { MetadataRoute } from 'next'
import { modules } from '@/data/modules'
import { articles } from '@/data/kennisbank/articles'
import { verticals } from '@/data/verticals'

const BASE_URL = 'https://doen.team'

const staticRoutes = ['', '/demo', '/demo-plannen', '/hoe-het-werkt', '/features', '/features/geheugen', '/prijzen', '/vergelijk', '/over', '/contact', '/kennisbank', '/veelgestelde-vragen', '/voorwaarden', '/privacy']

export default function sitemap(): MetadataRoute.Sitemap {
  return [
    ...staticRoutes.map((route) => ({
      url: `${BASE_URL}${route}`,
    })),
    ...modules.map((module) => ({
      url: `${BASE_URL}${module.href}`,
    })),
    ...verticals.map((vertical) => ({
      url: `${BASE_URL}/voor/${vertical.slug}`,
    })),
    ...articles.map((article) => ({
      url: `${BASE_URL}/kennisbank/${article.slug}`,
      lastModified: article.updatedAt,
    })),
  ]
}
