/**
 * Haalt omzet-grootboekrekeningen op uit de gekoppelde Moneybird-administratie,
 * voor de omzetrekening-select in Instellingen > Integraties.
 */
...
async function isRateLimited(key: string, maxCount: number, windowSeconds: number): Promise<boolean> {
  ...
  if (error) console.error('[moneybird-ledger-accounts] check_rate_limit faalde:', error)
  return data === true
}
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (await isRateLimited(`moneybird-ledger-accounts:${user_id}`, 30, 60)) {
    ...
    if (!mbRes.ok) {
      ...
      console.error('[moneybird-ledger-accounts] fout:', mbRes.status, body)
      return res.status(502).json({ error: `Moneybird gaf een fout (${mbRes.status}).` })
    }

    const accounts = await mbRes.json() as Array<{
      id: number | string
      name: string
      account_type: string
    }>
    ...
    return res.status(200).json(
      accounts
        ...
        .map((a) => ({ id: String(a.id), naam: a.name })),
    )
  } catch (err) {
    ...
    console.error('[moneybird-ledger-accounts] error:', message)
