...
async function getValidToken(userId: string): Promise<{ token: string; division: number }> {
  ...
  if (!refreshRes.ok) {
    ...
    if (refreshRes.status === 400 || refreshRes.status === 401) {
      ...
      if (errorBody.includes('invalid_grant')) {
        ...
        console.error('[Exact] invalid_grant — token rejected', {
          userId, endpoint: 'exact-btw-codes.ts', status: refreshRes.status,
        })
        Sentry.captureException(new Error('Exact invalid_grant'), {
          level: 'warning',
          tags: { exact_endpoint: 'exact-btw-codes', oauth_error: 'invalid_grant' },
    ...
    console.error('[Exact] token refresh fout', {
      endpoint: 'exact-btw-codes.ts', status: refreshRes.status, errorBody,
    })
  ...
  if (!tokens?.access_token) {
    console.error('[Exact] refresh gaf 200 zonder access_token', { endpoint: 'exact-btw-codes.ts' })
  ...
  if (updateFout) {
    console.error('[Exact] geroteerde token NIET opgeslagen — keten is hierna dood', {
      user_id: userId, endpoint: 'exact-btw-codes.ts', fout: updateFout.message,
    })
    Sentry.captureException(new Error('Exact token rotation not persisted'), {
      level: 'error',
      tags: { exact_endpoint: 'exact-btw-codes', oauth_error: 'rotation_not_persisted' },
    ...
    if (!nogGekoppeld) {
      console.warn('[Exact] rotatie niet bewaard: koppeling is inmiddels ontkoppeld', {
        user_id: userId, endpoint: 'exact-btw-codes.ts',
      })
    } else {
      ...
      if (insertFout && insertFout.code !== '23505') {
        console.error('[Exact] geroteerde token NIET opgeslagen (insert)', {
          user_id: userId, endpoint: 'exact-btw-codes.ts', fout: insertFout.message,
        })
        Sentry.captureException(new Error('Exact token rotation not persisted'), {
          level: 'error',
          tags: { exact_endpoint: 'exact-btw-codes', oauth_error: 'rotation_not_persisted' },
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const vatRes = await exactFetchMetRetry(
