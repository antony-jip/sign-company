...
async function isRateLimited(key: string, maxCount: number, windowSeconds: number): Promise<boolean> {
  ...
  if (error) console.error('[cancel-subscription] check_rate_limit faalde:', error)
  return data === true
}
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (await isRateLimited(`cancel-subscription:${user.id}`, 10, 3600)) {
    ...
    Sentry.captureException(error, { tags: { route: 'cancel-subscription' } })
