...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    // Check cache
    const cacheKey = `detail-${userId}-${code}`
    const cached = cache.get(cacheKey)
    if (cached && Date.now() < cached.expiresAt) {
      return res.status(200).json({ product: cached.data })
    }

    const response = await fetch(`${PROBO_BASE_URL}/products/product/${encodeURIComponent(code)}`, {
      headers: {
    ...
    if (response.status === 404) {
      return res.status(404).json({ error: `Product '${code}' niet gevonden` })
    }
    ...
    // Cache
    cache.set(cacheKey, { data: product, expiresAt: Date.now() + CACHE_TTL_MS })
