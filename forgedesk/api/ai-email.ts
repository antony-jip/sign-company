...
// Outreach krijgt een eigen regel (ruimer budget, adaptive thinking, web
// search), maar geen Opus meer: Sonnet 5 volstaat voor een mail van 120 woorden.
const MODEL_PER_ACTIE: Record<string, string> = {
  'write-lead-email': 'claude-sonnet-5',
}
...
if (!rlConfigured) {
  if (process.env.VERCEL_ENV === 'production') console.error('ratelimit niet geconfigureerd: api/ai-email.ts')
  else console.warn('[ratelimit] UPSTASH env vars missing for ai-email, requests will not be rate limited')
}
  ...
  try {
    ...
    const response = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        // het model zijn afwegingen soms in het zichtbare antwoord, en dat
    ...
    if (!response.ok) {
      ...
      if (response.status === 429) {
        ...
        if (anthropicType === 'enforced_spend_limit') {
          console.error('[anthropic-spend-limit] ai-email: Anthropic-budget van doen. bereikt')
