...
async function enforceRateLimit(identifier: string, res: VercelResponse): Promise<boolean> {
  ...
  try {
    ...
    console.warn(`[ratelimit-hit] ai-rewrite id=${identifier} limit=${limit}`)
    ...
    console.warn(`[ratelimit-error] ai-rewrite id=${identifier} err=${(err as Error).message}`)
    return true
  }
}
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    await schrijfSpoor(orgIdForBudget, userId, 'ai-rewrite', {
      actie: action,
      invoer: String(text).slice(0, 600),
