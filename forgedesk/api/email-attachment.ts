...
if (!rlConfigured) {
  console.warn('[ratelimit] UPSTASH env vars missing for email-attachment, requests will not be rate limited')
}
const ratelimit = rlConfigured
  ? new Ratelimit({ redis: Redis.fromEnv(), limiter: Ratelimit.slidingWindow(30, '60 s'), prefix: 'rl:email-attachment', timeout: 2000 })
  : null
...
async function enforceRateLimit(identifier: string, res: VercelResponse): Promise<boolean> {
  ...
  try {
    ...
    console.warn(`[ratelimit-hit] email-attachment id=${identifier} limit=${limit}`)
    ...
    console.warn(`[ratelimit-error] email-attachment id=${identifier} err=${(err as Error).message}`)
    return true
  }
}
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (all) {
      ...
      return res.status(200).json({
        attachments: payload.map(({ filename, contentType, size, content, isInlineCid }) => ({
          filename, contentType, size, content, isInlineCid,
        })),
      })
    }
