...
- Dashboard (/) : startpunt voor de dag. Blokken Vandaag (montages en taken) en Opvolgen (klanten die aandacht vragen), briefing van Daan, activiteitenlog, en een zwevende plusknop voor snelle acties.
...
- Cockpit (/cockpit, alleen beheerders) : één scherm met alles wat aandacht verdient (offertes die wachten of verlopen, facturen te laat, werkbonnen klaar om te factureren, portaal dat op de klant wacht, projecten zonder planning of over budget, inkoopfacturen op review), de cijfers per periode (pijplijn, gefactureerd, ontvangen, openstaand naar ouderdom, inkoop, marge, conversie; alles ex btw), wie er nu is ingeklokt, de live activiteitenfeed en de systeemstand (Exact-sync, herinneringen, Daan-nachtploeg, abonnement). Leveranciers staan onder /leveranciers.
...
${JSON.stringify(dataContext)}`
    ...
    const response = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        max_tokens: 6000,
        system: [
          // Dit blok is ~17k tokens productkennis. Een write van 1 uur kost 2x
          // in plaats van 1,25x en verdient zich pas terug bij drie vragen
          // binnen dat uur; bij het huidige gebruik (vragen liggen verder uit
          // elkaar dan een uur) is elke vraag toch een koude write en is de
          // standaard-TTL van 5 minuten simpelweg goedkoper. Gaat Daan vaker
          // gebruikt worden, zet dit op { ttl: '1h' } en CACHE_WRITE_FACTOR
          // mee op 2.
