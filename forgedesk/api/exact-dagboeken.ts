...
async function getValidToken(userId: string): Promise<{ token: string; division: number }> {
  ...
  if (updateFout) {
    console.error('[Exact] geroteerde token NIET opgeslagen — keten is hierna dood', {
      user_id: userId, endpoint: 'exact-dagboeken.ts', fout: updateFout.message,
    })
    Sentry.captureException(new Error('Exact token rotation not persisted'), {
      level: 'error',
      tags: { exact_endpoint: 'exact-dagboeken', oauth_error: 'rotation_not_persisted' },
    ...
    if (!nogGekoppeld) {
      console.warn('[Exact] rotatie niet bewaard: koppeling is inmiddels ontkoppeld', {
        user_id: userId, endpoint: 'exact-dagboeken.ts',
      })
    } else {
      ...
      if (insertFout && insertFout.code !== '23505') {
        console.error('[Exact] geroteerde token NIET opgeslagen (insert)', {
          user_id: userId, endpoint: 'exact-dagboeken.ts', fout: insertFout.message,
        })
        Sentry.captureException(new Error('Exact token rotation not persisted'), {
          level: 'error',
          tags: { exact_endpoint: 'exact-dagboeken', oauth_error: 'rotation_not_persisted' },
