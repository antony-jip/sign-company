...
// Vangnet voor het geval het id ooit naar een echte organisatie wijst (ander
// project, hersteld uit backup): zonder deze naam wordt er niets gewist.
const DEMO_ORG_NAAM = 'Signmakers Demo BV'
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    const { data: demoOrg, error: orgFout } = await supabaseAdmin
      .from('organisaties').select('naam').eq('id', DEMO_ORG).maybeSingle()
    if (orgFout || demoOrg?.naam !== DEMO_ORG_NAAM) {
      const melding = `Demo-reset afgebroken: organisatie ${DEMO_ORG} heet "${demoOrg?.naam ?? 'onbekend'}", niet "${DEMO_ORG_NAAM}"`
      console.error('[cron-demo-reset]', melding, orgFout ?? '')
      Sentry.captureMessage(melding, { level: 'error' })
      return res.status(500).json({ error: 'Demo-reset afgebroken: organisatie is niet de demo-organisatie' })
    }
