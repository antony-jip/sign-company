...
import { Upload, Download, Users, AlertCircle, AlertTriangle } from 'lucide-react'
import { useAuth } from '@/contexts/AuthContext'
import {
  parseCSV,
  valideerContactpersonen,
  importeerContactpersonen,
  generateContactpersonenTemplate,
  downloadCSV,
...
export function ContactpersonenUpload() {
  ...
  const handleFile = useCallback(async (file: File) => {
    ...
    try {
      ...
      const validatie = valideerContactpersonen(h, r)
  ...
  const handleImport = async () => {
    ...
    try {
      const result = await importeerContactpersonen(
        rows,
        organisatieId,
        user.id,
        fileName,
    ...
    <Card>
      <CardHeader>
        <CardTitle className="text-lg flex items-center gap-2">
          <Users className="w-5 h-5 text-[#1A535C]" />
          Contactpersonen
        </CardTitle>
        <p className="text-sm text-muted-foreground mt-1">
          Upload een CSV-bestand met je contactpersonen. Vul de bedrijfsnaam in om ze aan een klant te koppelen,
          of laat deze leeg voor losse contacten.
        </p>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        <Button
          variant="outline"
          size="sm"
          onClick={() => downloadCSV(generateContactpersonenTemplate(), 'contactpersonen_template.csv')}
          className="gap-2"
        >
        ...
        {rows.length > 0 && isValid && !resultaat && (
          <ImportPreview headers={headers} rows={rows} type="contactpersonen" />
        )}
        ...
        {isValid && rows.length > 0 && !resultaat && (
          <div className="space-y-3">
            ...
            >
              ...
              {importing ? 'Importeren...' : `Importeer ${rows.length.toLocaleString('nl-NL')} contactpersonen`}
            </Button>
          </div>
        )}

        {/* Result */}
        {resultaat && <ImportResultaatComponent resultaat={resultaat} type="contactpersonen" />}
        ...
        <ImportHelp type="contactpersonen" />
      </CardContent>
    </Card>
  )
}
