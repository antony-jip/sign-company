...
export function webfontImport(font: string): string {
  const optie = FONT_OPTIES.find(f => f.value === font && f.web)
  return optie?.web ? `@import url('https://fonts.googleapis.com/css2?family=${optie.web}&display=swap');` : ''
}

interface BlokBasis { id: string }

export interface KopBlok extends BlokBasis { type: 'kop'; tekst: string; niveau: 1 | 2 | 3; uitlijning: Uitlijning }
export interface TekstBlok extends BlokBasis { type: 'tekst'; html: string; uitlijning: Uitlijning; grootte: 'normaal' | 'klein' | 'groot' }
export interface AfbeeldingBlok extends BlokBasis { type: 'afbeelding'; url: string; alt: string; link: string; breedte: 'vol' | 'smal'; bijschrift: string }
export interface KnopBlok extends BlokBasis { type: 'knop'; tekst: string; url: string; uitlijning: Uitlijning; stijl: 'vol' | 'omlijnd'; breedte: 'auto' | 'vol' }
...
export interface KolommenBlok extends BlokBasis { type: 'kolommen'; kolommen: [Kolom, Kolom] }
...
export interface LijnBlok extends BlokBasis { type: 'lijn' }
...
function styleRichText(html: string, s: NieuwsbriefStijl, grootte: number, kleur: string, uitlijning: Uitlijning = 'links'): string {
  const lh = Math.round(grootte * 1.65)
...
function knopHtml(tekst: string, url: string, s: NieuwsbriefStijl, opties: { stijl?: 'vol' | 'omlijnd'; breedte?: 'auto' | 'vol'; uitlijning?: Uitlijning; klein?: boolean } = {}): string {
  if (!tekst?.trim()) return ''
  const { stijl = 'vol', breedte = 'auto', uitlijning = 'links', klein = false } = opties
  const vol = stijl === 'vol'
  const pad = klein ? '10px 20px' : '14px 30px'
  const fs = klein ? 14 : 15
  const td = vol
    ? `border-radius:8px;background:${s.accent};`
    : `border-radius:8px;border:2px solid ${s.accent};`
  const a = vol
    ? `display:inline-block;padding:${pad};font-family:${s.font};font-size:${fs}px;font-weight:700;color:#FFFFFF;text-decoration:none;border-radius:8px;`
    : `display:inline-block;padding:${pad};font-family:${s.font};font-size:${fs}px;font-weight:700;color:${s.accent};text-decoration:none;border-radius:8px;`
...
export function renderBlok(blok: Blok, s: NieuwsbriefStijl): string {
  switch (blok.type) {
    ...
    case 'kop': {
      ...
      return `<${tag} style="margin:0;font-family:${s.font};font-size:${maat}px;line-height:1.25;font-weight:800;letter-spacing:-0.02em;color:${s.tekst};text-align:${UITLIJN[blok.uitlijning]};">${escapeHtml(blok.tekst)}</${tag}>`
    }
    case 'tekst': {
      const maat = { klein: 13, normaal: 15, groot: 17 }[blok.grootte]
      return styleRichText(blok.html, s, maat, s.secundair, blok.uitlijning)
    }
    case 'afbeelding': {
      const w = blok.breedte === 'vol' ? CONTENT_BREEDTE : 400
      ...
      return `<table width="100%" cellpadding="0" cellspacing="0" role="presentation"><tr><td align="center">${afbeeldingHtml(blok.url, blok.alt, blok.link, String(w))}${bijschrift}</td></tr></table>`
    }
    case 'knop':
      return knopHtml(blok.tekst, blok.url, s, { stijl: blok.stijl, breedte: blok.breedte, uitlijning: blok.uitlijning })
    ...
    case 'kolommen': {
      const kol = (k: Kolom) => `<td class="stack" width="48%" valign="top" style="width:48%;padding:0;">
        ${k.url ? `<div style="margin-bottom:12px;">${afbeeldingHtml(k.url, k.kop, k.knopUrl, '257')}</div>` : ''}
      ...
      return `<table width="100%" cellpadding="0" cellspacing="0" role="presentation"><tr>${kol(blok.kolommen[0])}<td class="stack-gap" width="4%" style="width:4%;font-size:0;line-height:0;">&nbsp;</td>${kol(blok.kolommen[1])}</tr></table>`
    }
    case 'quote':
    ...
    case 'lijn':
      return `<table width="100%" cellpadding="0" cellspacing="0" role="presentation"><tr><td style="border-top:1px solid #EBEBEB;font-size:0;line-height:0;">&nbsp;</td></tr></table>`
    case 'ruimte':
...
export const RESPONSIVE_CSS = `@media only screen and (max-width:600px){ .stack{display:block!important;width:100%!important;padding-bottom:16px!important;} .stack-gap{display:none!important;} }`

export function renderDocument(doc: NieuwsbriefDocument): string {
  const s = doc.stijl
  return doc.blokken.map((b, i) => {
    const laatste = i === doc.blokken.length - 1
    const afstand = laatste ? 0 : (BLOK_AFSTAND[b.type] ?? 20)
    return `<!-- blok:${b.type} -->\n<div style="margin:0 0 ${afstand}px;">${renderBlok(b, s)}</div>`
  }).join('\n')
}
