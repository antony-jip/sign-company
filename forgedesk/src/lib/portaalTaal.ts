...
export interface PortaalTeksten {
  ...
  afmeting: string
  kortingKort: (pct: number) => string
  keuzeKop: string
  akkoordKnop: string
  pdfBestandsnaam: (nummer: string) => string
...
const NL: PortaalTeksten = {
  ...
  afmeting: 'Afmeting',
  kortingKort: (pct) => ` · ${pct}% korting`,
  keuzeKop: 'Kies zelf',
  akkoordKnop: 'Akkoord geven',
  pdfBestandsnaam: (nummer) => `Offerte-${nummer}.pdf`,
  ...
  keuzeHint: (n) => n === 1
    ? 'Bij één post kun je kiezen wat je wilt; het totaal rekent direct mee.'
    : `Bij ${n} posten kun je kiezen wat je wilt; het totaal rekent direct mee.`,
  bij: 'bij',
  ...
  akkoordPersoonlijk: (naam) => (naam ? `Akkoord geef je aan ${naam} persoonlijk, per mail of telefoon.` : 'Akkoord geef je ons persoonlijk, per mail of telefoon.'),
  vragenOverOfferte: 'Vragen over deze offerte?',
  helptGraag: (naam) => `${naam} helpt je graag verder.`,
  bedrijfHelpt: (bedrijf) => (bedrijf ? `${bedrijf} helpt je graag verder.` : 'We helpen je graag verder.'),
...
const FR: PortaalTeksten = {
  ...
  tekeningBekijken: 'Voir le plan',
  bijlage: (naam) => `Annexe : ${naam}`,
  afmeting: 'Dimensions',
  kortingKort: (pct) => ` · ${pct} % de remise`,
  keuzeKop: 'À vous de choisir',
  akkoordKnop: 'Accepter',
  pdfBestandsnaam: (nummer) => `Devis-${nummer}.pdf`,
  uitvoeringVan: (titel) => `Version de ${titel}`,
  uitvoering: 'Version',
  ...
  minstensEen: 'Au moins une version reste cochée.',
  toegevoegd: 'Ajouté à votre devis',
  toevoegen: 'Ajouter à votre devis',
  vraagOverPost: 'Question ou modification concernant ce poste',
  korting: 'Remise',
  kortingPct: (pct, bedrag) => `${pct} % (-${bedrag})`,
  offerte: 'Devis',
  ...
  nogDagen: (n) => ` (plus que ${n} ${n === 1 ? 'jour' : 'jours'})`,
  keuzeHint: (n) => n === 1
    ? 'Vous avez le choix pour un poste ; le total s\'adapte automatiquement.'
    : `Vous avez le choix pour ${n} postes ; le total s'adapte automatiquement.`,
  bij: 'chez',
  ...
  komtTerug: (voornaam) => (voornaam ? `${voornaam} reviendra vers vous` : 'Nous reviendrons vers vous'),
  danKomtTerug: (voornaam) => (voornaam ? `${voornaam} reviendra vers vous` : 'nous reviendrons vers vous'),
  ...
  contactPlanning: (voornaam) => (voornaam ? `${voornaam} vous contactera pour convenir du planning.` : 'Nous vous contacterons pour convenir du planning.'),
  bevestigingOnderweg: 'Une confirmation a été envoyée à ',
  ...
  akkoordUitleg: (bedrijf, metKeuze) => `En indiquant votre nom et en signant, vous acceptez ce devis${bedrijf ? ` de ${bedrijf}` : ''}${metKeuze ? ', avec le choix indiqué ci-dessus' : ''}.`,
  akkoordOndertekenen: 'Accepter et signer',
  bevestigingPerMail: 'Vous recevrez immédiatement une confirmation par e-mail.',
  akkoordGeven: 'Donner votre accord',
  akkoordPersoonlijk: (naam) => (naam ? `Vous donnez votre accord directement à ${naam}, par e-mail ou par téléphone.` : 'Vous nous donnez votre accord directement, par e-mail ou par téléphone.'),
  ...
  verlopenNieuweVersie: (komtTerug) => `Ce devis a expiré. ${komtTerug} avec une nouvelle version.`,
  ...
  opnieuw: 'Effacer',
  standaardAkkoordIntro: 'Votre nom, votre signature, et nous nous mettons au travail.',
  standaardBedanktKop: 'Merci pour votre commande',
  standaardBedanktTekst: 'Dès que votre commande sera traitée, vous recevrez une confirmation.',
}
