...
function buildClientEmailHtml(p: ClientEmailParams): string {
  ...
  const logoHtml = p.logoUrl
    ? `<img src="${escapeHtml(p.logoUrl)}" alt="${escapeHtml(p.bedrijfsnaam)}" style="max-height: 48px; max-width: 200px; object-fit: contain;" />`
    : `<span style="font-family: -apple-system, BlinkMacSystemFont, sans-serif; font-size: 22px; font-weight: 700; color: #1A1A1A;">${escapeHtml(p.bedrijfsnaam)}</span>`
  ...
  const itemBlock = p.itemTitel ? `
    <tr><td style="padding: 0 0 16px 0;">
      <table width="100%" cellpadding="0" cellspacing="0" style="border: 1px solid #EBEBEB; border-radius: 8px;">
        <tr><td style="padding: 16px 20px; font-family: -apple-system, BlinkMacSystemFont, sans-serif; font-size: 15px; font-weight: 600; color: #1A1A1A;">
          ${escapeHtml(p.itemTitel)}
        </td></tr>
        ${p.beschrijving ? `<tr><td style="padding: 0 20px 16px 20px; font-family: -apple-system, BlinkMacSystemFont, sans-serif; font-size: 14px; color: #6B6B66; line-height: 1.6;">${escapeHtml(p.beschrijving)}</td></tr>` : ''}
      </table>
    </td></tr>` : ''

  const quoteBlock = p.quote ? `
    <tr><td style="padding: 0 0 20px 0;">
      <table width="100%" cellpadding="0" cellspacing="0" style="background-color: ${colorLight}; border-radius: 8px; border-left: 4px solid ${color};">
        <tr><td style="padding: 16px 20px; font-family: -apple-system, BlinkMacSystemFont, sans-serif; font-size: 14px; color: #1A1A1A; font-style: italic; line-height: 1.6;">
  ...
  const ctaBlock = p.ctaUrl ? `
    <tr><td style="padding: 8px 0 0 0;" align="center">
      <a href="${escapeHtml(p.ctaUrl)}" target="_blank" style="display: inline-block; background-color: ${color}; color: #FFFFFF; font-family: -apple-system, BlinkMacSystemFont, sans-serif; font-size: 15px; font-weight: 600; text-decoration: none; padding: 14px 32px; border-radius: 8px; line-height: 1;">
...
<body style="margin: 0; padding: 0; background-color: #F4F3F0; -webkit-font-smoothing: antialiased;">
  <table width="100%" cellpadding="0" cellspacing="0" style="background-color: #F4F3F0; padding: 40px 20px;">
    <tr><td align="center">
      <table width="100%" cellpadding="0" cellspacing="0" style="max-width: 520px;">
        <tr><td style="padding: 0 0 24px 0; text-align: center;">${logoHtml}</td></tr>
        <tr><td>
          <table width="100%" cellpadding="0" cellspacing="0" style="background-color: #ffffff; border-radius: 12px; box-shadow: 0 2px 16px rgba(0,0,0,0.04);">
            <tr><td style="padding: 36px 36px 32px 36px;">
              <table width="100%" cellpadding="0" cellspacing="0">
                ...
                ${itemBlock}${quoteBlock}
                <tr><td style="padding: 16px 0 0 0; font-family: -apple-system, BlinkMacSystemFont, sans-serif; font-size: 14px; color: #6B6B66; line-height: 1.8;">Met vriendelijke groet,<br><strong style="color: #1A1A1A;">${escapeHtml(p.bedrijfsnaam)}</strong></td></tr>
                ${ctaBlock}
              </table>
            </td></tr>
          </table>
        ...
        <tr><td style="padding: 20px 0 0 0; text-align: center;">
          <span style="font-family: -apple-system, BlinkMacSystemFont, sans-serif; font-size: 11px; color: #9B9B95;">Verzonden via <span style="font-weight: 700;">doen</span><span style="color: #F15025; font-weight: 700;">.</span></span>
        </td></tr>
      </table>
    </td></tr>
  </table>
