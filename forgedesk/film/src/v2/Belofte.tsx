...
export const Belofte: React.FC<Props> = ({ t, op, uit, tekst, kernwoord, selectOp, positie = 'boven', punt = true }) => {
  ...
  const size = lang ? 72 : 92
  return (
    <div style={{ position: 'absolute', left: 72, right: 72, ...(positie === 'boven' ? { top: 330 } : { bottom: 560 }), zIndex: 40, opacity: zicht, transform: `translateY(${(1 - inP) * 30}px)`, fontFamily: fonts.kop, fontWeight: 700, fontSize: size, lineHeight: 1.04, letterSpacing: '-0.035em', color: merk.petrol, textWrap: 'balance' as never }}>
      {delen[0]}
